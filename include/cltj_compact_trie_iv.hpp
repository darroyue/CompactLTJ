        ...
        // public:
            ...
            */
            ...
            const uint64_t subtree_size(uint64_t it){
                ...
                //cout << " subtree_size it=" << it << endl;
                uint64_t leftmost_leaf, rightmost_leaf;

                //cout << " subtree_size leftmost_leaf is " << child(child(it, 1), 1) << " and is obtained passing by node " << child(it, 1) << endl;
                leftmost_leaf = child(child(it, 1), 1);
                it = child(it, n_children);
                //cout << " subtree_size it is now " << it; 

                n_children = childrenCount(it);
                rightmost_leaf = child(it, n_children);
                //cout << " and rightmost_leaf is " << rightmost_leaf << endl;
            ...
            */
            ...
            // NEW DIEGO
            // Given a position within seq, gets its key
            uint32_t get_key_at(uint32_t pos) {
                return seq[pos];
            }

            pair<uint32_t, uint32_t> binary_search_seek(uint32_t val, uint32_t i, uint32_t f){
                if (seq[f] < val) return make_pair(0,f+1);
                /*if (seq[i] > val) return make_pair(seq[i], i); // NEW DIEGO  
                uint64_t step = 1;
		while (i+step <= f && seq[i+step] < val) {
		    step <<= 1;
		}
                i = i + (step>>1);*/
                uint32_t mid; 
